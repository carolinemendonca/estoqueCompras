...
public class Server {
    ...
    private ServerSideConnection idCliente1;
    private ServerSideConnection idCliente2;
    ...
    public void acceptConnections() {
        try {
            ...
            while (numPlayers < 2) {
                ...
                numPlayers++;
                System.out.println("Cliente "+numPlayers+" conectado!");
                ServerSideConnection serverSideConnection = new ServerSideConnection(socket, numPlayers);
                if (numPlayers == 1) {
                    idCliente1 = serverSideConnection;
                } else {
                    idCliente2 = serverSideConnection;
                }
                Thread thread = new Thread(serverSideConnection);
                thread.start();
            }
            System.out.println("Limite de clientes conectados. ");
        } catch (IOException ex) {
            System.out.println(ex);
